...
\begin{enumerate}[(a)]
    % Question 1-A
    \item \points{2a} First, implement the |sigmoid()| function in \texttt{src/submission.py} to apply the sigmoid function to an input vector. In the same file, fill in the implementation for the |naive\_softmax\_loss\_and\_gradient()| function. Then, fill in the implementation of the loss and gradient functions for the skip-gram model (|skipgram()|).\newline
    ...
    % Question 1-B
    \item \points{2b} Complete the implementation for your SGD optimizer in the |sgd()| function in |src/submission.py|.
